...
# 核心修改：删除price_speed因子
FEATURE_CONFIG = [
    'price_vol_corr_pos',              # 正向化量价相关性
    'lastprice_vol_converge',          # 最新价波动收敛率
    'vol_volatility',                  # 成交量波动率
    'return_volatility_pos',           # 正向化收益率波动率
    'short_vol_ratio',                 # 年化交易量比率
    'daily_rel_turnover',              # 日相对换手率
    'buy_depth_ratio_enhanced',        # 强化版买盘深度占比
    'e_vs_sector_depth_diff_enhanced'  # 自适应平滑板块差异因子
]
FEATURE_DIM = len(FEATURE_CONFIG)
...
def calculate_vol_volatility(vol_series, window=TICK_PER_5MIN):
    ...
    vol_std_standardized = standardize_feature(vol_std)
    return clean_numeric_array(vol_std_standardized)
...
def calculate_return_volatility_pos(return_series, window=TICK_PER_5MIN):
    ...
    vol_pos = np.abs(vol_series) * (-1)
    vol_pos_standardized = standardize_feature(vol_pos)
    return clean_numeric_array(vol_pos_standardized)
...
def calculate_daily_rel_turnover(vol_series, share_cap=1e8):
    ...
    rel_turnover_standardized = standardize_feature(rel_turnover)
    return clean_numeric_array(rel_turnover_standardized)
...
def calculate_buy_depth_ratio_enhanced(e_data):
    ...
    enhanced_ratio = scaler.fit_transform(smooth_ratio.reshape(-1, 1)).flatten()
    return clean_numeric_array(enhanced_ratio)
...
def calculate_e_vs_sector_depth_diff_enhanced(sector_data, e_data):
    ...
    smooth_window = ONLINE_SMOOTH_WINDOW if diff_std > 0.05 else 1200
    smooth_diff = pd.Series(depth_diff).rolling(window=smooth_window, min_periods=1).mean().values
    enhanced_diff = smooth_diff * 2.0
    return clean_numeric_array(enhanced_diff)
...
def calculate_batch_features(day_data):
    ...
    return features, labels, single_ic_results
